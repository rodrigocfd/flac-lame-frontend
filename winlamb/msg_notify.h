...
protected:
	msg_notify()
	{
		on_message(WM_NOTIFY, [this](params p)->typename traitsT::ret_type {
			params_notify pn(p);
			return _callbacks.process(hwnd(), WM_NOTIFY,
...
public:
	...
	{
		_callbacks.add(std::make_pair(idFrom, code), std::move(callback));
	}

	void on_notify(std::initializer_list<std::pair<UINT_PTR, UINT>> idFromAndCodes, func_notify_type callback)
	{
		_callbacks.add(idFromAndCodes, std::move(callback));
	}
};
