...
public:
	...
	DC&      textOut(int x, int y, const std::wstring& text)          { return textOut(x, y, text.c_str(), text.length()); }
	...
	SIZE     getTextExtent(const std::wstring& text)                                 { return getTextExtent(text.c_str(), text.length()); }
...
// For within WM_PAINT, double-buffered; you must return zero on WM_ERASEBKGND message.
