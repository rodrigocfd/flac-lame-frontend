...
impl MappedFile {
	...
	pub fn open(
		file_path: &str,
		access: MappedFileAccess) -> Result<MappedFile, Box<dyn Error>>
	{
	...
	fn map_in_memory(&mut self) -> Result<(), Box<dyn Error>> {
	...
	/// recreated.
	pub fn resize(&mut self, num_bytes: usize) -> Result<(), Box<dyn Error>> {
