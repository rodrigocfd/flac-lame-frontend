...
struct sysdlg final {
	...
	static int msgbox(const base_wnd* parent, std::wstring title, std::wstring text, UINT uType = 0) {
	...
	static bool open_file(const base_wnd* parent, const wchar_t* filterWithPipes, std::wstring& buf) {
	...
	static bool open_file(HWND hWnd, const wchar_t* filterWithPipes, std::vector<std::wstring>& arrBuf) {
		...
		if (GetOpenFileNameW(&ofn)) {
			...
			if (!strs.size()) {
				DBG(L"ERROR: GetOpenFileName didn't return multiple strings.\n");
				return false;
			}
		...
		DBG((errNo == FNERR_BUFFERTOOSMALL) ?
			L"ERROR: GetOpenFileName buffer is too small.\n" :
			str::format(L"ERROR: GetOpenFileName failed with error %u.\n", errNo).c_str() );
		return false;
	}

	static bool open_file(const base_wnd* parent, const wchar_t* filterWithPipes, std::vector<std::wstring>& arrBuf) {
	...
	static bool save_file(const base_wnd* parent, const wchar_t* filterWithPipes, std::wstring& buf, const wchar_t* defFile) {
	...
	static bool choose_folder(const base_wnd* parent, std::wstring& buf) {
