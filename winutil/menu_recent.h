...
public:
	menu_recent();
	menu_recent& init(HWND hWnd, size_t subMenuIndex, WORD subMenuItemCmdId, winutil::file_ini& iniFile, const std::wstring& iniSection, size_t maxEntries = 9);
	std::wstring get_entry(size_t index) const;
	menu_recent& add_entry(const std::wstring& newPath);
