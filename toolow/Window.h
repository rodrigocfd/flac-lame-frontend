...
public:
	...
	Window& setTextFmt(const wchar_t *fmt, ...) {
		va_list args; va_start(args, fmt);
		String buf; buf.fmtv(fmt, args);
		va_end(args); setText(buf.str()); return *this;
	}

	wchar_t* getText(wchar_t *pBuf, int szBuf) { ::GetWindowText(_hWnd, pBuf, szBuf); return pBuf; }
...
protected:
	...
	int    messageBox(UINT uType, const wchar_t *caption, const wchar_t *fmt, ...);
