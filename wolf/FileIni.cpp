...
bool FileIni::load(const wchar_t *file, wstring *pErr)
{
	...
	wstring content;
	if (!fm.getContent(content, 0, -1, pErr)) return false;
	...
	for (auto& line : lines) {
		...
		if (line[0] == L'[' && line.back() == L']') { // begin of section found
			...
			this->data.emplace(curSection, unordered_map<wstring, wstring>()); // new section added
		} else if (!this->data.empty() && !line.empty() && line[0] != L';') { // keys will be read only if within a section
			...
			if (idxEq != wstring::npos) {
				...
				this->data[curSection].emplace(keyBuf, valBuf);
			}
		}
	}
	return true;
...
{
	return this->data.find(section) != this->data.end();
}

bool FileIni::hasKey(const wchar_t *section, const wchar_t *key) const
{
	if (!this->hasSection(section)) return false;
	return this->data.at(section).find(key) != this->data.at(section).end();
}