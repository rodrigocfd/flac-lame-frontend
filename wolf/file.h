...
public:
	...
	static bool showOpen(const Window *wnd, const wchar_t *filter, std::wstring& buf);
	static bool showOpen(HWND hWnd, const wchar_t *filter, std::vector<std::wstring>& arrBuf);
	static bool showOpen(const Window *wnd, const wchar_t *filter, std::vector<std::wstring>& arrBuf);
	static bool showSave(HWND hWnd, const wchar_t *filter, std::wstring& buf, const wchar_t *defFile);
	static bool showSave(const Window *wnd, const wchar_t *filter, std::wstring& buf, const wchar_t *defFile);
	static bool showChooseFolder(HWND hWnd, std::wstring& buf);
	static bool showChooseFolder(const Window *wnd, std::wstring& buf);
